...
public class DrawShapes
{
    ...
    {
        Circles, Squares, Text, Random, RandomWithText
    }
    #endregion
    ...
    /// <param name="centerTextPanel"></param>
    public DrawShapes(Form form, Panel centerTextPanel, ShapeType shape)
    {
        this.centerTextPanel = centerTextPanel;
        mainForm = form;
        shapeType = shape;
        Initialise();
    }
    ...
    /// <param name="centerTextPanel"></param>
    public DrawShapes(Form form, Panel centerTextPanel, string shape)
    {
        this.centerTextPanel = centerTextPanel;
        mainForm = form;
    ...
    public void Draw(string str)
    {
        switch (shapeType)
        {
            ...
            case ShapeType.RandomWithText:
                if (!String.IsNullOrEmpty(str))
                    DrawRandomWithText(str);
                else
                    DrawRandom();
                break;
    ...
    private void DrawRandomWithText(string str)
    {
        //first draw some random shape
        DrawRandom();

        //next, show the typed text in the middle of the screen untill the next letter is typed
        Font font;
        Brush colourBrush;
        int intFontSize, X, Y;
        SizeF sizeText;
        ...
        using (var centerGraphics = centerTextPanel.CreateGraphics())
        {
            //clear old text
            centerGraphics.Clear(backColor);
            ...
            do
            {
                ...
                sizeText = centerGraphics.MeasureString(str, new Font(RandomFont.Name, intFontSize, RandomFont.DefaultStyle));
            } while (sizeText.Width <= centerTextPanel.ClientRectangle.Width && sizeText.Height <= centerTextPanel.ClientRectangle.Height);

            //one smaller, and now we are sure the text fits exactly in the panel
            intFontSize--;

            // and coordinates
            X = (int)(centerTextPanel.ClientRectangle.Width - sizeText.Width) / 2;
            Y = (int)(centerTextPanel.ClientRectangle.Height - sizeText.Height) / 2;
            
            colourBrush = new SolidBrush(GetRandomColor());

            // Draw the text to the form and the bitmap.
            centerGraphics.DrawString(str, font, colourBrush, X, Y);
        }
    }
